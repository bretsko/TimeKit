...
public extension DayMoment {
    ...
    static func translate(hour: Int,
        ...
        let lid1 = LID(lid)
        guard let ruleSet = lid1.dayMomentRuleSet,
              let moment = ruleSet.moment(for: hour) else {
            return nil
        }
        return moment.translate(lid)
    }
    
    
    //MARK: -
    
    func translate(_ lid: LIDP = LID.current) -> Str? {
        // let lid1 = LID(lid)
    ...
    //TODO: consider changing to [Str] - in some langs can be several variants
    func translate(_ lid: LangLID) -> Str? {
