...
public extension TimeFormatter {
    ...
    struct Style {
        ...
        public init(_ translStyles: [TimeTranslStyle] = [.long],
                    _ ruleSet: [Rule] = TimeFormatter.convenientRuleSet,
        ...
        public static var defaultAllowedUnits: [Time] {
    ...
    struct Rule {
        ...
        typealias CustomFormatter = ((Date) -> (String))
    ...
    //        return Style([.long_time], .convenient)
    //    }
    
    //    static var twitterStyle: Style {
    //        return Style([.tiny, .short_time, .narrow, .short_time],
    //                     .twitter)
    //    }
}
...
public extension TimeFormatter {
    ...
    /// Canonical ruleSet
    static var canonicalRuleSet: [Rule] {
        return [
            Rule(Moment.now,
                 threshold: .value(0)),
            ...
            Rule(AnyPeriod.week,
                 threshold: .value(6.5 * DurationUnit.day.factor)),
            
            Rule(AnyPeriod.month,
                 threshold: .value(3.5 * 7 * DurationUnit.day.factor)),
            
            Rule(AnyPeriod.year,
                 threshold: .value(1.5 * DurationUnit.month.factor))
        ]
    }
    
    /// Convenient ruleSet
    static var convenientRuleSet: [Rule] {
        return [
            Rule(Moment.now,
                 threshold: .value(0)),
            ...
            Rule(AnyPeriod.day,
                 threshold: .value((20.5 / 24) * DurationUnit.day.factor)),
            
            Rule(AnyPeriod.week,
                 threshold: .value(5.5 * DurationUnit.day.factor)),
            
            Rule(AnyPeriod.month,
                 threshold: .value(3.5 * 7 * DurationUnit.day.factor)),
            
            Rule(AnyPeriod.year,
                 threshold: .value(10.5 * DurationUnit.month.factor))
        ]
    }
    
    /// Twitter ruleSet
    static var twitterRuleSet: [Rule] {
        return [
            Rule(Moment.now,
                 threshold: .value(0)),
