...
public extension Date {
    ...
    var firstDayOfWeek: Int? {
        return date(at: .startOfWeek)?.day
    }
    
    var lastDayOfWeek: Int? {
        return date(at: .endOfWeek)?.day
    }
    ...
    func date(at weekdayOrdinal: Int,
              _ weekday: WeekDay,
              monthNumber: Int? = nil,
    ...
    func date(at dayOfMonth: Int,
              monthNumber: Int? = nil,
              yearNumber: Int? = nil) -> Date? {
        return Date(year: yearNumber ?? year,
                    month: monthNumber ?? month,
                    day: dayOfMonth,
                    hour: hour,
                    minute: minute,
    ...
    func dateAfter(weeksCount: Int,
                   on weekday: WeekDay) -> Date? {
        
        guard var r = adding(weeksCount, of: .weekOfMonth) else {
            return nil
        }
        ...
        } else if newWeekDay.index > weekday.index,
    ...
    func next(_ weekday: WeekDay,
              ...
              andMonthNumber monthNumber: Int? = nil) -> Date? {
        
        guard let result = date(at: weekNumber,
                                weekday,
        ...
        if let monthNum = monthNumber {
            ...
            return date(at: weekNumber, weekday,
    ...
    //    static func now(at relPeriod: RelDatePeriod) -> Date? {
    //        return Date().date(at:relPeriod)
    //    }
    ...
    static func dates(for weekday: WeekDay,
        ...
        let weekdayOffset = weekday.index
        ...
        var dateOccurrences = [firstOccurrence]
        while true {
            
            let nextDate = calendar.date(byAdding: DateComps(day: 7),
                                         to: dateOccurrences.last!)!
