...
public extension Month {
    ...
    var allCasesSorted: [Int] {
        Month.allCases.sorted().map{$0.int}
    }
    ...
    func adding(months: Int) -> Month? {
        ...
        return Month((int + normalized + 12) % 12)
    }
    ...
    /// Subtracts a number of months from the current month and returns the new month. May be negative, in which case it will be added
    func subtracting(months: Int) -> Month? {
        adding(months: -(months % 12))
    }
