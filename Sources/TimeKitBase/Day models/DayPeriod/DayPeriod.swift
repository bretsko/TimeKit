...
public enum DayPeriod: Str, StrEnumT {
...
public extension DayPeriod {
    ...
    func dayInterval(in locale: LID = .current) -> DayInterval {
        fatalError()
    }
    
    
    /// if locale param is nil uses current locale
    //func period(for hour: Int, in locale: LID?) -> DayPeriod
    
    //MARK: convs
    
    var isMorning: Bool {
        self == .morning1 || self == .morning2
    }
    var isAfternoon: Bool {
        self == .afternoon1 || self == .afternoon2
    }
    var isEvening: Bool {
        self == .evening
    }
